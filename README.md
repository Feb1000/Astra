# 💀 DoomShame Extension (v2.0)

> **The Ultimate Anti-Doomscroll Inconvenience Engine & Screen Time Roast Tracker**

DoomShame is a Google Chrome extension (Manifest V3) designed to track your active browsing time and roast you with custom audio soundboards, meme overlays, and desktop notifications whenever you hit screen time milestones.
...
- **Coward Ambush System**: If you try to pause screen time tracking, DoomShame ambushes you with an instant coward roast modal and audio callout!
...
5. Pin the **DoomShame** extension icon to your browser toolbar.

---

## 🚀 How to Test 1-Minute Trigger

1. Open the **DoomShame** popup window from the Chrome toolbar.
...
├── background.js        # Core background service worker & tracking engine
├── content.js           # In-page roast overlay injection engine
├── popup.html           # Sleek control panel UI
├── popup.js             # Control panel timer logic & fast controls
├── options.html         # Custom Trigger Studio UI
├── options.js           # Trigger Studio rule builder & file uploader
└── assets/              # Meme GIFs, SVGs, and MP3 soundboard files
```
...
- `storage`: Preserves screen time state, rule thresholds, and custom soundboard media.
- `tabs`: Interacts with active web tabs for overlay delivery.
- `scripting`: Programmatically injects `content.js` on pages missing content scripts.
- `alarms`: Enforces heartbeat background time tracking.
- `notifications`: Sends Chrome Desktop alerts when triggers fire.
- `idle`: Detects user system activity and pauses tracking when AFK.
